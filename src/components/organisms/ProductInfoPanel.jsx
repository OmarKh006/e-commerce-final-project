...
export default function ProductInfoPanel({ product }) {
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl font-medium mb-2">{product.title}</h1>
      ...
      {product.colors?.length > 0 && (
        <div className="flex items-center gap-4 mb-4">
          ...
          <ColorSwatchPicker colors={product.colors} selected={color} onChange={setColor} />
        </div>
      )}
      ...
      {product.sizes?.length > 0 && (
        <div className="flex items-center gap-4 mb-6">
          ...
          <SizePicker sizes={product.sizes} selected={size} onChange={setSize} />
        </div>
      )}

      <div className="flex items-center gap-4 mb-8">
        <QuantityStepper value={qty} onChange={setQty} />
        <Button variant="primary" size="lg" onClick={handleBuyNow} disabled={!product.inStock}>
          {t('product.buyNow')}
        </Button>
        <button
        ...
        >
          <FiHeart className={isWishlisted ? 'fill-primary text-primary' : 'text-ink'} size={18} />
        </button>
      </div>
      ...
      <div className="border border-line rounded-sm">
        <div className="flex items-start gap-4 p-4 border-b border-line">
          ...
          <div>
            ...
            <p className="text-xs text-body underline">{t('product.freeDeliveryDesc')}</p>
          </div>
        </div>
        <div className="flex items-start gap-4 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-body">{t('product.returnDeliveryDesc')}</p>
          </div>
        </div>
      </div>
    </div>

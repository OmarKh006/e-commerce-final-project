  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-24">
        ...
        <Button as={Link} to="/" variant="primary">{t('cart.returnToShop')}</Button>
      </div>
    )
  }
      ...
      {items.map((item) => (
        ...
        >
          <div className="flex items-center gap-4 col-span-2 md:col-span-1">
            <img src={item.product.image} alt={item.product.title} className="w-14 h-14 object-cover rounded-sm bg-secondary-gray" />
            <span className="text-sm">{item.product.title}</span>
          </div>
          <span className="text-sm">${item.product.price}</span>
          <QuantityStepper size="sm" value={item.qty} onChange={(qty) => updateQty(item.key, qty)} />
